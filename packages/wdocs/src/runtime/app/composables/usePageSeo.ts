...
export const usePageSeo = (page: Ref<PageData | null | undefined>) => {
  ...
  const title = computed(() => page.value?.frontmatter?.title || siteName.value)
  const favicon = computed(() => page.value?.frontmatter?.favicon || '/favicon.ico')
  ...
  useSeoMeta({
    ...
    ogDescription: page.value?.frontmatter?.['og:description'] || page.value?.frontmatter?.description || defaultDescription.value,

...
const _isActive = computed(() => route.path === props.item.path)

const _paddingClass = computed(() => `pl-[${0.5 + props.depth * 0.75}rem]`)
</script>
...
<template>
  <li>
    <NuxtLink
      :to="item.path"
      class="block text-sm rounded-md transition-colors duration-200 py-1.5"
      :class="[
        _paddingClass,
        _isActive
          ? 'bg-primary-100 dark:bg-primary-900/50 text-primary-600 dark:text-primary-400 font-semibold'
